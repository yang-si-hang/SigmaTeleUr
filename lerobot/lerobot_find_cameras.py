...
def find_all_orbbec_cameras() -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("Skipping Orbbec camera search: lerobot_camera_orbbec library not found or not importable.")
    except Exception as e:
...
def create_camera_instance(cam_meta: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        elif cam_type == "Orbbec":  # Orbbec cameras report types like "OBDeviceType.***"
            orbbec_config = OrbbecCameraConfig(
                serial_number_or_name=cam_id,
                fps=cam_meta.get("default_stream_profile", {}).get("fps"),
                width=cam_meta.get("default_stream_profile", {}).get("width"),
                height=cam_meta.get("default_stream_profile", {}).get("height"),
                color_mode=ColorMode.RGB,
            )
